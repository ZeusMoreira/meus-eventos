<template>
  ...
  />
  <div class="listagem">
    <h2 class="listagem__titulo">Últimos Eventos</h2>
    <ul class="listagem__lista" v-if="eventos.length > 0">
      <li
        class="listagem__lista__card"
        v-for="(evento, index) in eventos"
        :key="index"
        @click="tratarEventoAcessarEvento(evento)"
      >
        ...
        <div class="card__participantes-infos">
          <i class="mdi mdi-account card-icone"></i>
          <span class="card-quantidade-partipantes">{{ evento.quantidadeParticipantes }}</span>
        </div>
      </li>
    </ul>
    <div v-else class="no-data">
      ...
      <span>Clique em <b>Gerenciar Eventos</b>.</span>
    </div>
  </div>
</template>
...
const emit = defineEmits(['acessar-evento'])

function tratarEventoAcessarEvento(evento) {
...
.card-quantidade-partipantes {
  font-size: 1.4rem;
  font-weight: normal;
}

.card__participantes-infos {
  display: flex;
  align-items: center;
}
